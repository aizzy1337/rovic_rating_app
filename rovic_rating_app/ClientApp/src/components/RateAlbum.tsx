...
export default function RateAlbum() {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        fetch("https://localhost:44426/api/album/search/" + data.get('text'), {
          method: "GET",
          headers: {
            "Authorization": "Bearer " + token,
          },
    ...
    const handleAddSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        await fetch('https://localhost:44426/api/album/', {
      method: 'POST',
      headers: {
        "Authorization": "Bearer " + token,
      ...
      body: JSON.stringify({
        ...
        productionYear: albums[0].productionYear,
    ...
    .then((data) => {
       ...
       tagList.forEach(async tag => {
        await fetch('https://localhost:44426/api/album/' + data.id + '/tag/' + tag, {
            method: 'POST',
            headers: {
              "Authorization": "Bearer " + token,
              "Content-Type": "application/json"
                ...
                {albums.slice(0,1).map((album, index) => {
                    return (
                        <Grid key={index} item xs={12} justifyContent="center" alignItems="center">
                            ...
                            >
                                    <img src={album.cover} alt={album.title} height="400"/>
                                    <div style={{
                                        margin: 50
                                    }}>
                                    ...
                                    <Box component="form" onSubmit={handleAddSubmit} noValidate sx={{ mt: 1 }} flexDirection={"column"}>
                                        <Select
                                            ...
                                            multiple
                                        >
                                            {albumTags.slice(0,4).map((tag, index) => {
                                                return (
